...
<template>
  <nav
    ...
    <div
      ...
      :class="isScrolled ? 'backdrop-blur-md bg-background/80 border-b border-white/5 opacity-100' : 'opacity-0'"
    ></div>

    <div class="relative max-w-7xl mx-auto px-6 lg:px-12 flex items-center justify-between">
      ...
      <div class="hidden lg:flex items-center gap-10 z-10">
        <a
          ...
          <div class="absolute bottom-0 left-0 w-full h-px bg-accent scale-x-0 origin-right transition-transform duration-300 ease-out group-hover:scale-x-100 group-hover:origin-left"></div>
        </a>
      </div>
      ...
      <button
        ...
        <div class="w-6 h-5 flex flex-col justify-between">
          <span class="w-full h-0.5 bg-current transition-all duration-300 origin-left" :class="isMobileOpen ? 'rotate-45' : ''"></span>
          <span class="w-full h-0.5 bg-current transition-all duration-300" :class="isMobileOpen ? 'opacity-0' : ''"></span>
          <span class="w-full h-0.5 bg-current transition-all duration-300 origin-left" :class="isMobileOpen ? '-rotate-45' : ''"></span>
        </div>
      </button>
    </div>
    ...
    <div
      ...
      <a
        ...
        :style="{ transform: isMobileOpen ? 'translateY(0)' : 'translateY(20px)', opacity: isMobileOpen ? 1 : 0, transitionDelay: `${100 + index * 50}ms`, transitionDuration: '500ms' }">
        {{ link.label }}
      </a>
    </div>
  </nav>
